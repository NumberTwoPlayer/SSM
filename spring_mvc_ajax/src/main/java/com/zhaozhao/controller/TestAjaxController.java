 ...
 * @Date 2023/8/16 13:49
 * 1、@RequestBody：将请求体中的内容和控制器方法的形参进行绑定
 *
 * 2、使用@RequestBody注解将json格式的请求参数转换为Java对象
 * a>导入jackson的依赖
 * b>在springmvc的配置文件中设置 <mvc:annotation-driven/>
 * c>在处理请求的控制器方法的形参位置，直接设置json格式的请求参数要转换的Java类型的形参，使用@RequestBody注解表示即可
 *
 * 3、@ResponseBody：将所标识的控制器方法的返回值作为相应报文的相应体的浏览器
 *
 * 4、使用@ResponseBody注解响应浏览器json格式的数据
 * a>导入jackson的依赖
 * b>在springmvc的配置文件中设置 <mvc:annotation-driven/>
 * c>在处理请求的控制器方法的形参位置，直接设置json格式的请求参数要转换的Java类型的形参，使用@ResponseBody注解标识控制器方法
