...
/**
 * @author 刘梹晨
 * @Classname TestRequesMappingController
 ...
 * 作用：通过请求的请求路径匹配请求
 * value属性是数组类型，即当前浏览器所发送请求的请求路径所匹配value属性中的任何一个值
 * 则当前请求就会被注解所标识的方法进行处理
 *
 * 3、@RequestMapping注解的method属性
 * 作用：通过请求的请求方式匹配请求
 * method属性是RequestMethod类型的数组，即当前浏览器所发送请求的请求方式所匹配method属性中的任何一种请求方式
 * 则当前请求就会被注解所标识的方法进行处理
 * 若浏览器所发送的请求的请求路径和@RequestMapping注解value属性匹配，但是请求方式不匹配，
 * 此时页面报错：HTTP状态 405 - ***不允许
 ...
 * 此时页面报错：HTTP状态 400 - Parameter conditions "username, !password" not met for actual request parameters: username={admin}, password={}
 *
 ...
 * ？：任意的单个字符（不包括?）
 * *：任意个数的任意字符（不包括?和/）
 * **：任意层数的目录，注意使用方式只能**写在双斜线中，前后不能有任何其他字符
 *
...
@Controller
//@RequestMapping("/test")
public class TestRequesMappingController {
    //此时控制器方法所匹配的请求的请求1路径为/test/hello
    @RequestMapping (
            value = {"/hello","/abc"},
